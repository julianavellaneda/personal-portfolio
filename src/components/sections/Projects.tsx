...
const projects: Project[] = [
  ...
  {
    title: 'WARDEN',
    description: 'Reverse-tactics roguelike in Rust + Bevy — you play the dungeon boss, programming minions against heroes who react to whatever plan you commit.',
    imageUrl: '/projects/warden/cover.png',
    demoUrl: 'https://bugz0213.itch.io/warden',
    // TODO: uncomment once the repo is flipped public.
    // repoUrl: 'https://github.com/julianavellaneda/warden-mini-jam-212',
    technologies: ['Rust', 'Bevy 0.18', 'WebAssembly', 'bevy_ecs_tilemap', 'Python'],
    fullDescription: 'A turn-based tactics roguelike built for Mini Jam 212 (theme: Control, limitation: You are the enemy). Heroes storm your keep to reach your Core. You can\'t move freely — each turn you program your minions from a limited shared rack of action tiles, commit the plan, then watch it resolve register-by-register against heroes who adapt to what you just locked in. The core loop shipped inside the 72-hour jam window; the roguelike meta layer around it was built in the week after.',
    problem: 'Every tactics game casts you as the clever hero reacting to scripted enemies. WARDEN inverts it: you are the scripted side, but you write the script. Your power isn\'t flexibility — it\'s foresight.',
    role: 'Solo developer — game design, engine work, the procedural art/audio pipeline, and the web build.',
    built: 'A Bevy 0.18 ECS game on an 8×8 grid with a phase state machine (Planning → Execution → HeroResolution → CheckWin). Drag-to-program minions from one shared tile pool, a timer-stepped register sweep so the commit animates, and hand-rolled BFS pathing for hero AI that always telegraphs its intent. On top of that sits a full roguelike meta loop — floor map, rewards and deck-building, shop/forge economy, wards, narrative events, minion roster and deploy, elite and boss encounters, run recap. Every sprite, UI icon, sound effect and music loop is generated by a deterministic Python pipeline (Pillow + NumPy) rather than hand-drawn, and a screenshot harness boots the real app into any named screen for byte-identical visual regression checks. ~27k lines of Rust behind ~249 tests.',
    learned: 'Data-oriented ECS design under jam pressure, how much leverage a deterministic visual-verification harness gives you when iterating fast, and that generating assets in code beats sourcing them when the clock is the real constraint.',
    challenges: 'The whole game rests on telegraphing: heroes must react intelligently to a committed plan without ever invalidating the intent the player already saw. Anything that secretly reroutes a path turns perfect-information tactics into guesswork. Scoping was the other half — a commit-and-resolve loop is only fun once it is completely legible.',
    solutions: 'Resolution is a register-by-register, timer-stepped sweep so every consequence is readable as it happens. Hidden hazards are resolution-only and never enter the grid used for hero pathing, so a telegraphed route can be openly interrupted but never silently rewritten. Grid logic was kept render-agnostic, which made a late visual pass a coordinate swap instead of a rewrite.',
    screenshots: [
      '/projects/warden/planning.png',
      '/projects/warden/floormap.png',
      '/projects/warden/resolve-hit.png',
    ],
  },
];
